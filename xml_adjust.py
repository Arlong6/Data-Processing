...
original_path = r"D:\Data\Pascal2012\VOCdevkit\VOC2012"
target_path = r"E:\orange"
old_classes = ["orangre"]  # 需要的類別
new_classes = "orange"  # 改新的類別
...
def search_file():
    ...
    for file_name in os.listdir(os.path.join(original_path, "Annotations")):
        ...
        # 打開xml文件進行解析
        in_file = open(old_ann_path, encoding="utf-8",errors="ignore")
...
def search_person_vehicle():
    ...
    for file_name in os.listdir(os.path.join(target_path, "Annotations")):
        ...
        print(file_path)
        in_file = open(file_path, encoding="utf-8",errors="ignore")
        tree = ET.parse(in_file, parser = ET.XMLParser(encoding = 'iso-8859-5'))  # ET是一个xml文件解析庫，ET.parse（）打开xml文件。parse--"解析"
        root = tree.getroot()  # 獲取根節點
